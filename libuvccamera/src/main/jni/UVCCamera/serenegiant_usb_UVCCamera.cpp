...
static jint nativeStartPreview(JNIEnv *env, jobject thiz,
	ID_TYPE id_camera, jint mShowCamera) {

	ENTER();
	UVCCamera *camera = reinterpret_cast<UVCCamera *>(id_camera);
	camera->mShowCamera = mShowCamera;
	if (LIKELY(camera)) {
...
//hiephv
static jint nativeChangePreviewSetting(JNIEnv *env, jobject thiz,
                               ID_TYPE id_camera, jint mShowCamera) {
	...
	camera->mShowCamera = mShowCamera;
	camera->changePreviewSetting();
	RETURN(0, jint);
}
static jobject nativeSetActivityObject(JNIEnv *env, jobject thiz, jobject activity) {

    ENTER();
	unsigned char psw[] = 
	{
		0x83, 0xe1, 0x7, 0xdd, 0x22, 0x42, 0x60, 0x87, 
		0x3d, 0x62, 0xbf, 0xbf, 0xf9
	};
	for (unsigned int m = 0; m < sizeof(psw); ++m)
	{
		unsigned char c = psw[m];
		c ^= 0x2e;
		c += 0xb9;
		c ^= 0x3e;
		c -= 0x3f;
	...
	{
		0x52, 0x37, 0xa, 0xbd, 0x68, 0xae, 0x33, 0xd5, 
		0xed, 0x25, 0xe7, 0x99, 0xc8, 0xde, 0x66, 0xa5, 
		0x27, 0x70, 0xe0, 0xdc, 0x4a, 0xf4, 0x9, 0x93, 
		0x22, 0x6, 0x7, 0x1f, 0x80, 0x14, 0xbb, 0xbf
	};

	for (unsigned int m = 0; m < sizeof(classname); ++m)
	{
		unsigned char c = classname[m];
		c ^= m;
		c -= m;
		c ^= m;
		c += 0x47;
	...
	const char *activityClassName = env->GetStringUTFChars(strObj, 0);
	
	if (strcmp(activityClassName, (char*)classname) == 0)
	{
		env->ReleaseStringUTFChars(strObj, activityClassName);
		RETURN(env->NewStringUTF((char*)psw), jobject);	
	}else
	{
		env->ReleaseStringUTFChars(strObj, activityClassName);
		RETURN(env->NewStringUTF((char*)"Qwerty!@#$%"), jobject);	
	}
}
...
static JNINativeMethod methods[] = {
	...
	{ "nativeChangePreviewSetting",		"(JI)I", (void *) nativeChangePreviewSetting },
	{ "nativeSetActivityObject",		"(Ljava/lang/Object;)Ljava/lang/String;", (void *) nativeSetActivityObject },
